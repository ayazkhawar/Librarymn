...
#creating table
        ...
        list1 = Menu(self)
        list1.add_command(label="Add Student", command=a_s)
        list1.add_command(label="Remove Student", command=r_s)
        ...
        self.mymenu.add_cascade(label='Manage', menu=list1)
        ...
        def check():
            try:
                ...
                if not z:
                    ...
                    #label and input box
                    self.label3 = Label(self, text=' LIBRARY MANAGEMENT SYSTEM',fg='sea green',bg="pale green" ,font=('Times New Roman', 30, 'bold'))
                    self.label3.place(x=300, y=22)
                    self.label4 = Label(self, text="ENTER THE STUDENT ID:",bg="pale green", fg='sea green', font=('Courier new', 18, 'bold'))
                    self.label4.place(x=75, y=107)
                    self.studid = Entry(self, textvariable=self.a, width=90)
                    self.studid.place(x=405, y=110)
                    self.srt = Button(self, text='Search', width=15,bg="sea green",fg='white', font=('arial', 10),command = ser).place(x=1000, y=106)
                    self.label5 = Label(self, text="ENTER THE BOOK ID:",bg="pale green",fg='sea green', font=('Courier new', 18, 'bold'))
                    self.label5.place(x=75, y=150)
                    self.bookid = Entry(self, textvariable=self.b, width=90)
                    self.bookid.place(x=405, y=160)
                    self.brt = Button(self, text='Find', width=15,bg="sea green",fg='white', font=('arial', 10),command = ent).place(x=1000, y=150)
                    self.label6 = Label(self, text="INFORMATION DETAILS",bg="pale green",fg='sea green',  font=('Courier new', 15, 'underline', 'bold'))
                    self.label6.place(x=560, y=310)
                    self.button = Button(self, text='Search Student', width=25,bg="sea green",fg='white', font=('Courier new', 10), command=sest).place(x=240,y=250)
                    self.button = Button(self, text='Search Book', width=25,bg="sea green",fg='white', font=('Courier new', 10), command=sea).place(x=520,y=250)
                    self.brt = Button(self, text="Issue Book", width=15,bg="sea green",fg='white', font=('Courier new', 10), command=ib).place(x=800, y=250)
                    self.brt = Button(self, text="Return Book", width=15,bg="sea green",fg='white', font=('Courier new', 10), command=ret).place(x=1000, y=250)
                    self.brt = Button(self, text="LOGOUT", width=15,bg="green",fg='white', font=('Courier new', 10), command=log).place(x=1150, y=105)
            except Error:
                messagebox.showerror("Error", "Something Goes Wrong")
        check()
