...
class Search(Tk):
    def __init__(self):
        ...
        self.canvas = Canvas(width=800, height=500, bg='pale green')
        self.canvas.pack()
        self.iconbitmap(r'libico.ico')
        l1=Label(self,text="SEARCH BOOKS",fg='sea green',bg='pale green', font=("times new roman",20,'bold')).place(x=290,y=40)
        l = Label(self, text="SEARCH BY",fg='sea green',bg='pale green', font=("Courier new", 15, 'bold')).place(x=180, y=100)
        def insert(data):
        ...
        def ge():
            if (len(g.get())) == 0:
                ...
                try:
                    ...
                    messagebox.showerror("Error","Something goes wrong")
        b=Button(self,text="Find",width=15,fg='white', bg='sea green',font=("Courier new",10,'bold'),command=ge).place(x=400,y=170)
        c=ttk.Combobox(self,textvariable=g,values=["Book Name","Author Name","Book Id"],width=40,state="readonly").place(x = 310, y = 105)
        en = Entry(self,textvariable=f,width=43).place(x=310,y=145)
        la = Label(self, text="ENTER",fg='sea green', bg='pale green', font=("Courier new", 15, 'bold')).place(x=180, y=140)
